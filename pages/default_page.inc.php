 ...
 */
	if ( isset($_POST['function']) and ($_POST['function']=='save') ) {
		...
		if (($fh = fopen($rxa_slimbox['path'].'/'.$rxa_slimbox['name'].'.ini', 'w')) === FALSE) {
			$rxa_slimbox['meldung'] = $rxa_slimbox['i18n']->msg('error_save',$rxa_slimbox['path'].'/'.$rxa_slimbox['name'].'.ini');
		} else {
			@fwrite($fh, $line);
			@fclose($fh);	
			$rxa_slimbox['meldung'] = $rxa_slimbox['i18n']->msg('msg_saved');
		}
	}
 ...
 */
	if (($lines = file($rxa_slimbox['path'].'/'.$rxa_slimbox['name'].'.ini')) === FALSE) {
		$rxa_slimbox['meldung'] = $rxa_slimbox['i18n']->msg('error_read',$rxa_slimbox['path'].'/'.$rxa_slimbox['name'].'.ini');
	} else {
		$va = explode(',', trim($lines[0]));
		$allcats = trim($va[0]);
		$subcats = trim($va[1]);
		$rxa_slimbox['cats'] = unserialize(trim($lines[1]));
	}
	if (!is_array($rxa_slimbox['cats'])){
		$rxa_slimbox['cats'] = array();
	}
 ...
 */
   ...
   $select_cats = "\n";
	if ($cats = OOCategory::getRootCategories())
	{
		foreach( $cats as $cat)
		{
			add_cat_options($rxa_slimbox, $select_cats, $cat, $cat_ids);
		}
	}
	//Artikel aus dem Root ebenso in die Auswahl (selectbox) übernehmen
	add_rootart_options($rxa_slimbox, $select_cats, $clang);

	$selsize = $rxa_slimbox['catselectcount'] / 3;
...
<form action="index.php?page=<?php echo $rxa_slimbox['name']; ?>" method="post">
...
<?php
	if ($rxa_slimbox['meldung']<>'') echo $rxa_slimbox['meldung'].'<br />';
	echo $rxa_slimbox['i18n']->msg('text_settings_intro');
?>
        <br /><br />

        <input type="checkbox" id="allcats" name="allcats" value="1" <?php if ($allcats == "1") echo "checked"; ?> />
		  <label for="allcats"><?php echo $rxa_slimbox['i18n']->msg('text_settings_allcats'); ?></label>
        <br />
        
        <input type="checkbox" id="subcats" name="subcats" value="1" <?php if ($subcats == "1") echo "checked"; ?> />
		  <label for="subcats"><?php echo $rxa_slimbox['i18n']->msg('text_settings_subcats'); ?></label>
        <br /><br />

<?php echo $rxa_slimbox['i18n']->msg('text_settings_help'); ?>

<?php
	echo $select_cats;
?>
        <br /><br />
        <input type="submit" value="<?php echo $rxa_slimbox['i18n']->msg('button_save'); ?>" />
